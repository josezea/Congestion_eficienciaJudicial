##################### TIPO DE DESPACHO: Juzgado de Circuito ####################
...
# df <- df_2019;  Anno <- "2019";  vctr_especialidad = "Familia"
...
f_analisis <- function(df, Anno, vctr_especialidad){
  ...
  df_JuridiccCompetenEspecial <- df %>% filter(JURISDICCIÓN == "Ordinaria" & 
                                                 `TIPO DE DESPACHO` == "Juzgado de Circuito", 
                                               ESPECIALIDAD == vctr_especialidad) %>%
    group_by(JURISDICCIÓN, 
             `TIPO DE DESPACHO`, DISTRITO, CIRCUITO, ESPECIALIDAD) %>% 
 ...
 # dfRes_JuridCompet$CIRCUITO_EFICIENCIA <- paste0(dfRes_JuridCompet$CIRCUITO, " (Eficiencia = ", round(dfRes_JuridCompet$eficiencia, 2), ")")
 
  dfRes_JuridCompet$DISTCIRCEFIC <- paste0("D: ", dfRes_JuridCompet$DISTRITO, 
                                          " , C: ", dfRes_JuridCompet$CIRCUITO, 
                                       ", Eficiencia:", round(dfRes_JuridCompet$eficiencia, 2))
  # Interactivo
   ...
   graficoInteractivo_egresosVSunidades_comp <- plot_ly(data = dfRes_JuridCompet, x = ~NUM_UNIDADES, 
          ...
          text = ~DISTCIRCEFIC) %>% layout(tickformat = "digit" , separators= ".",
                                                  title = paste0("Análisis juzgados de circuito (Juridicción: ordinaria - Especialidad: ", 
  ...
  # dfResIncom_JuridCompet$CIRCUITO_EFICIENCIA <- paste0(dfResIncom_JuridCompet$CIRCUITO, " (Eficiencia = ", 
  ...
  graficoInteractivo_egresosVSunidades_incomp <- plot_ly(data = dfResIncom_JuridCompet, x = ~NUM_UNIDADES, 
    ...
    dfRes_JuridCompet$CIRCUITO <- fct_rev(forcats::fct_inorder(dfRes_JuridCompet$CIRCUITO)) 
    # dfRes_JuridCompet$CIRCUITO_EFICIENCIA <- paste0(dfRes_JuridCompet$CIRCUITO, " (Eficiencia = ", 
    #                                                      round(dfRes_JuridCompet$eficiencia, 2), ")")
    # 
    graficoInteractivo_eficiencia <- plot_ly(data = dfRes_JuridCompet, x = ~CIRCUITO, 
                                                           y = ~eficiencia,
  ...
  dfRes_JuridCompet$CIRCUITO <- fct_rev(forcats::fct_inorder(dfRes_JuridCompet$CIRCUITO)) 
  dfRes_JuridCompet$DescongEfic <- paste0("D: ", dfRes_JuridCompet$DISTRITO, 
                                          " , C: ", dfRes_JuridCompet$CIRCUITO,
                                            ", Descongestión = ",
  ...
  graficoInteractivo_eficienciaIdeal <- plot_ly(data = dfRes_JuridCompet, x = ~CIRCUITO, 
    ...
    layout(tickformat = "digit" , separators= ".", 
           title = paste0("Análisis juzgados de circuito (Juridicción: ordinaria - Especialidad: ", 
                          tolower(vctr_especialidad), ")"),
           xaxis = list(title = "Circuitos", titlefont = list(family = "Courier New, monospace", 
                                                              size = 18, color = "#7f7f7f"), tickformat = "digits"),
  ...
  dfRes_JuridCompet$CIRCUITO_INEFICIENCIA <- NULL
  ...
  names(dfRes_JuridCompet) <- c("Juridicción", "Tipo de despacho", "Distrito", "Circuito", "Especialidad", "Número de jueces",
                                "Número de juzgados de circuito", "Ingresos efectivos", "Egresos Efectivos", "Total de inventario inicial", "Total de inventario final", "Cargas",
                                "Descongestión", "Grupo", "Eficiencia", "Ineficiencia", "Congestión", "Congestión escenario ideal", 
  ...
  carpeta <- paste0("year_", Anno, "_Circuito_","Juridiccion", unique(dfRes_JuridCompet$Juridicción), "_", "Especialidad", 
